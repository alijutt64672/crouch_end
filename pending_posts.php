...
$requests = mysqli_query($conn,"SELECT request.id,request.service_provider_id,request.client_id,request.to_date,request.request_type, users.name AS client_name, services.service_name, request.description, request.address, request.rating, request.amount, request.request_time, request.request_date, request.time_consumed FROM request INNER JOIN users ON request.client_id = users.id INNER JOIN services ON request.service_id = services.id WHERE request.status = 0 ORDER BY request.id DESC");
$counter = 0;
